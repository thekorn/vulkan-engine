...
pub fn init(alloc: std.mem.Allocator, device: *Device, extend: c.VkExtent2D, previousSwapChain: ?*Self) !Self {
    const createSwapChainResult = try createSwapChain(alloc, device, extend, previousSwapChain);
...
fn createSwapChain(alloc: std.mem.Allocator, device: *Device, extend: c.VkExtent2D, previousSwapChain: ?*Self) !CreateSwapChainResult {
    ...
    var createInfo = c.VkSwapchainCreateInfoKHR{
        ...
        .clipped = c.VK_TRUE,
        .oldSwapchain = if (previousSwapChain) |p| p.swapChain else null,
    };

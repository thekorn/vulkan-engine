...
the C module generated by `build.zig`'s `addTranslateC` step.

It boils down to **what Zig's C translator can translate** vs. **what the
...
## Zig's C translator only handles C

`addTranslateC` runs the input through Zig's **C translator**. It only
understands C declarations. When it hits C++ constructs
— templates, `std::vector`, name-mangled overloads, RTTI, exceptions —
it either skips them or fails outright. There's no way to say
"translate `tiny_obj_loader.h`" and get back something usable from Zig.
...
`extern "C"`), so Zig can translate it cleanly and sees
...
1. Adds this directory to the include path so the translation step can find
   `tinyobj_wrapper.h`:
...
The build writes a small umbrella header, translates all C-ABI
prototypes alongside the GLFW and Vulkan headers, and imports the
result as `c`. [`src/c.zig`](../../c.zig) re-exports that module:
...
- **Use a pure-C OBJ loader instead** — e.g.
  ...
  separate project. Zig's C translator could consume that header
...
layer** that gives Zig something its C translator can parse, and
